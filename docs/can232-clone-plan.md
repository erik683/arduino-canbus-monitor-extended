...
Last updated: `c0b7e7496e3ea1ae57f17d018ede2aa51aaca9a5`

## Repository Snapshot (pre-work)

````text
HEAD: c0b7e7496e3ea1ae57f17d018ede2aa51aaca9a5
...
6. **Acceptance Code/Mask (`Mxxxxxxxx`, `mxxxxxxxx`)**  
...
8. **Timestamp Consistency (`Z`)**  
   - Already extended with `Z2`; keep it but default to standard behavior.  
   - Ensure `Z` implicitly reports current mode when invoked without argument (CAN232 quirk).  
   - Persist to EEPROM alongside `Q` so power-cycle retains timestamp mode.
...
1. Land shared EEPROM helper.  
2. Implement metadata + `F` updates (low risk).  
3. Move sequentially through command groups (`Q`, `W/M/m`, `s`).  
4. Keep CANUSB backlog (`docs/canusb-future-goals.md`) updated as new insights surface.
