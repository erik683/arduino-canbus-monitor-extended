#!/usr/bin/env python3
"""Interactive diagnostics sampler for quick manual checks."""
...
def test_diagnostics(port: str | None, baud: int) -> int:
    ...
    commands = [
        ("Version (V)", b"V\r"),
        ("Serial number (N)", b"N\r"),
        ("CAN status before opening (F)", b"F\r"),
        ("Set bitrate to 125 kbps (S4)", b"S4\r"),
        ("Open CAN bus (O)", b"O\r"),
        ("CAN status after opening (F)", b"F\r"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    return test_diagnostics(args.port, args.baud)
